...
import { getRepostableJobs } from "utils/server/jobs";
import sanatizeJob from "utils/server/sanatize-job";
import { buildMetadata } from "app/shared-metadata";

export const generateMetadata = buildMetadata({ title: "Expired Jobs" });

export default async function ExpiredJobs() {
  const rawJobs = await getRepostableJobs();
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        <div className="py-4">
          ...
          <JobsTable jobs={jobs} filter="expired" />
        </div>
      </div>
    </main>
  );
